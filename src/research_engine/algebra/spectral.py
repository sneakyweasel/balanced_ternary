"""Companion classification. Floats are labels only, never acceptance."""
...
def exact_pisot_cubic_certificate(coeffs: tuple[int, ...]) -> dict[str, object]:
    ...
    product of roots ``-c``. Floating roots are not used.
    """
    if len(coeffs) != 4 or coeffs[0] != 1:
...
def cubic_roots(coeffs: tuple[int, ...]) -> tuple[complex, complex, complex]:
    """Roots of ``x^3 + a x^2 + b x + c``. Classification labels, not a proof."""
    if len(coeffs) != 4 or coeffs[0] != 1:
...
def is_monic_cubic(coeffs: tuple[int, ...]) -> bool:
    return len(coeffs) == 4 and coeffs[0] == 1
