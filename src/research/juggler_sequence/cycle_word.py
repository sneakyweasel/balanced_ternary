...
def search_cycles(word: str, n_max: int) -> list[int]:
    ...
    for n in range(2, n_max + 1):
        if follows_word(n, word) and image_after(n, word) == n:
            hits.append(n)
    return hits
...
def lean_api_present() -> dict[str, bool]:
    ...
    for name in LEAN_THEOREMS:
        if name == "CycleWord":
            named[name] = "def CycleWord" in text
        else:
...
def classify(scan: dict[str, Any], lean: dict[str, bool]) -> dict[str, Any]:
    lean_ok = (
        ...
        and lean["no_cycle_word_odd"]
        and lean["no_cycle_word_oo"]
        and lean["no_cycle_word_eoo"]
    ...
    return {
        ...
        "reason": (
            ...
            "contracting words, O, OO, and EOO are excluded; "
