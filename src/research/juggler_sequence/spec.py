"""Hint-free one-variable integer map with even/odd floor-power successors.

Exposes only the exact successor when the integer is positive and inside
the declared bit budget. No literature names, halt claims, or affine hint.
"""
...
class FloorPowerSpec:
    ...
    def __post_init__(self):
        ...
        if not self.name:
            object.__setattr__(self, "name", "juggler")
...
def map_spec(*, start: int = 13) -> FloorPowerSpec:
    return FloorPowerSpec(start=start, name="juggler")
