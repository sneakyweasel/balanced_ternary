...
on a CycleWord, the endpoint quantities Delta and T/n^P. The exact
...
from research.juggler_sequence.global_defect import follows_word, image_after, odd_count
...
def cycle_endpoint_defect(n: int, odd: int, length: int) -> int:
    """Delta on a CycleWord: n^{3^o} - n^{2^L}. Independent of letter order."""
...
def cycle_normalized_exponent(odd: int, length: int) -> tuple[int, int]:
    """Exponent of T/n^P on a CycleWord: (2^L - 3^o)/2^L."""
...
def endpoint_quantities_word_free(n: int, left: str, right: str) -> bool:
    """Same (o, L) ⇒ same CycleWord endpoint quantities, ignoring the word."""
    ...
    for n in range(lo, hi):
        for length in range(1, k_max + 1):
            ...
            for mask in range(1 << length):
                ...
                if other != word and follows_word(n, other):
                    return False
    return True
...
def classify(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if unarchived == 0 and triple_ok and contracting_ok and collapse:
        ...
        decision = "CLOSE"
        reason = (
            "on a CycleWord the endpoint quantities are (o, L)-only; "
...
def probe_payload() -> dict[str, Any]:
    ...
    payload = {
        ...
        "note": (
            "Same (o, L) fixes 3^o/2^L and, on a CycleWord, Delta and "
