...
from research.home_prime_49.planner import plan_map_session, plan_strategy
from research.home_prime_49.scout import BASELINE
from research.home_prime_49.spec import FactorConcatSpec, map_spec
...
def _yield_report(summary: SessionSummary, spec: FactorConcatSpec) -> dict[str, Any]:
    ...
    return {
        ...
        "new_exact_results": "T(7)=7; seed 4 reaches 211; packet seed 49 maps to 77 then truncates",
        "new_invariants": "none; a budget-truncated prefix is not a Z-theorem",
        "new_obstructions": "none; no residue-affine cover",
        "new_origin_reachability_results": evidence["steps_to_prime"],
        "new_nonreachability_results": "none claimed on integers >= 2",
...
def _representation_failure() -> FailureRecord:
    return FailureRecord(
        id="home_prime:representation",
        target="home_prime",
        experiment_id="home_prime_49",
        ...
        evidence="factor concatenation has no complete piecewise-affine cover on the sample window",
        reusable_lesson=(
            "Factor concatenation is outside residue-affine language; a budget-truncated "
            "seed-49 prefix is not aliquot truncation, not a prime-reachability theorem, "
            "and not a dedicated concatenation attack."
        ),
        ...
        minimal_example="n |-> concat(prime factors of n)",
    )
...
def run_campaign(corpus: ResearchCorpus | None = None) -> tuple[ResearchCorpus, CampaignReport]:
    ...
    extra = {
        ...
        "mathematical_yield": MathematicalYield(
            known_rediscoveries=(
                "packet seed 49 maps to 77 then grows until the factorization cap",
                "no complete piecewise-affine cover",
            ),
            new_exact_results=("T(7)=7", "4 maps to 22 then 211"),
            new_formalizations=("Problems.Engine.FactorConcat",),
            new_obstructions=("none; a truncated prefix is not a prime-reachability theorem",),
            new_counterexamples=("T(49)=77 is not aliquot or floor-power; T(8)=222 is not reverse-add",),
            unresolved_questions=("whether seed 49 reaches a prime",),
            engineering_changes=0,
        ),
        "grey_loot": (
            GreyLoot(
                id="home_prime:loot:mismatch",
                kind=GreyLootKind.REPRESENTATION_MISMATCH,
                statement="factor concatenation lies outside residue-affine census language",
            ...
            GreyLoot(
                id="home_prime:loot:seed49",
                kind=GreyLootKind.USEFUL_NEGATIVE_RESULT,
                statement="seed 49 maps to 77 then truncates at the factorization cap; that is not a theorem that 49 reaches a prime",
                evidence=LootEvidence.OBSERVED,
                experiment_id="home_prime_49",
                target="home_prime",
            ),
            GreyLoot(
                id="home_prime:loot:cluster",
                kind=GreyLootKind.USEFUL_NEGATIVE_RESULT,
                statement="engine EXACT_CLOSURE of the truncated prefix is a budget artefact, distinct from aliquot UNBOUNDED_SAMPLE and from reverse-add/juggler attractor closures",
                evidence=LootEvidence.OBSERVED,
                experiment_id="home_prime_49",
                target="home_prime",
            ),
    ...
    experiment = experiment_from_session(
        ...
        scout=ScoutDossier(
            ...
            problem_definition=(
                "Does frozen v2.3 diagnose factor concatenation without a dedicated attack or a prime-reachability theorem?"
            ),
