"""Hint-free one-variable integer map n |-> sigma(n) - n.

Exposes only the exact successor when factorization succeeds inside the
declared budget. No literature names, cycle classifications, or affine hint.
"""
...
def map_images(n: int) -> tuple[int, ...]:
    ...
    if factors is None:
        return ()
    return (sigma_from_factors(factors) - n,)


@dataclass(frozen=True)
class SigmaMinusNSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name:
            object.__setattr__(self, "name", f"sigma_minus_n_{self.start}")
...
def map_spec(*, start: int = 276) -> SigmaMinusNSpec:
    return SigmaMinusNSpec(start=start)
