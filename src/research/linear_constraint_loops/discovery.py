...
def step(spec: OneVariableLoopSpec, x: int) -> int | None:
...
def orbit(spec: OneVariableLoopSpec, start: int, *, max_steps: int | None = None) -> tuple[int, ...]:
...
def empirical_termination(spec: OneVariableLoopSpec, window: tuple[int, ...] = WINDOW) -> dict[str, int]:
    stopped = 0
    cyclic = 0
    truncated = 0
    for seed in window:
...
def magnitude_census(spec: OneVariableLoopSpec, window: tuple[int, ...] = WINDOW) -> dict[str, int]:
    drops = 0
    growths = 0
    equals = 0
    undefined = 0
...
def falsify_claims(spec: OneVariableLoopSpec) -> dict[str, dict[str, object]]:
    mag = magnitude_census(spec)
    cycles = short_cycles(spec)
    term = empirical_termination(spec)
    ...
    return {
        "monotone_descent": {
            ...
            "counterexample": None if monotone else _first_growth(spec),
            "census": mag,
        },
        "no_nontrivial_cycle": {
        ...
        "empirical_termination": {
            "claim": "every window seed stops",
            "holds_on_window": all_stop,
            "census": term,
        },
        "one_step_contraction": {
            "claim": "every defined image satisfies |y| < |x| for x != 0",
            "holds_on_window": _contraction_holds(spec),
            "counterexample": _first_noncontraction(spec),
        },
    }


def _first_growth(spec: OneVariableLoopSpec) -> int | None:
    for seed in WINDOW:
        image = step(spec, seed)
...
def _contraction_holds(spec: OneVariableLoopSpec) -> bool:
    return _first_noncontraction(spec) is None


def _first_noncontraction(spec: OneVariableLoopSpec) -> int | None:
    for seed in WINDOW:
        if seed == 0:
            continue
        image = step(spec, seed)
