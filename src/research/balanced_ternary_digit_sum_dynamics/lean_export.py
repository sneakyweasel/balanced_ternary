"""Link digit-sum certificates to Lean names. Does not generate proofs."""
...
DIGIT_SUM_MODULE = "Problems.BalancedTernary.DigitSumDynamics"
CLOSURE_THEOREM = "digitSumZ_natAbs_lt"
        ...
        if (
            ...
            if size == 3:
                out.append(
                    attach_lean(
                        target,
                        module=DIGIT_SUM_MODULE,
                        theorem=CLOSURE_THEOREM,
                        name="digit_sum_contraction",
                    )
                )
                continue
        out.append(target)
...
def export_digit_sum_targets(report: PlannerReport) -> tuple[TheoremTarget, ...]:
    return link_digit_sum_targets(
        targets_from_report(report, problem="balanced_ternary_digit_sum_dynamics")
    )
