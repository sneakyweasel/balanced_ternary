"""Integer iterates of the balanced-ternary digit-square map.

State is the integer ``n``. The adapter does not install an attractor,
Hamming-weight name, modulus, or Lyapunov.
"""
...
def digit_square_sum(n: int) -> int:
    """``T(n) = W(n)`` by exact local digits: ``W(n) = lsd(n)² + W(D(n))``."""
...
class WeightDynamicsSpec:
    """Integer dynamics of ``T(n)=W(n)`` with identity observation."""

    start: int = DEFAULT_START
    start_remaining: int = INPUT_LENGTH
    state_cap: int = INTEGER_STATE_CAP
    box_bound: int = BOX_BOUND
    name: str = "balanced_ternary_weight_dynamics"
    dimension: int = 1
    ...
    def transition(self, state: State, control: object, phase: IntPhase) -> State:
        del control, phase
        return (digit_square_sum(state[0]),)
    ...
    def attack_context(self, **kwargs) -> AttackContext:
        nxt = digit_square_sum(self.start)
...
def weight_dynamics_spec(
    start_remaining: int = INPUT_LENGTH,
    start: int = DEFAULT_START,
) -> WeightDynamicsSpec:
    return WeightDynamicsSpec(start=start, start_remaining=start_remaining)
