"""Link weight-dynamics certificates to Lean names. Does not generate proofs."""
...
WEIGHT_MODULE = "Problems.BalancedTernary.WeightDynamics"
CLOSURE_THEOREM = "weightZ_natAbs_lt"
    ...
    for target in targets:
        if (
            ...
            for cert in target.certificates:
                if isinstance(cert, dict) and cert.get("size") == 2:
                    size = 2
            if size == 2:
                out.append(
                    attach_lean(
...
def export_weight_targets(report: PlannerReport) -> tuple[TheoremTarget, ...]:
    return link_weight_targets(
        targets_from_report(report, problem="balanced_ternary_weight_dynamics")
    )
