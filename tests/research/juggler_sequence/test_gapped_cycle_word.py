"""Gapped three-even CycleWord leftovers. Not an engine-control or halt test."""

from __future__ import annotations

import json

from research.juggler_sequence.gapped_cycle_word import (
    ALLOWED,
    CLASS_GREEN,
    FORBIDDEN,
    LEAN_THEOREMS,
...
def test_lean_api_and_classify():
    ...
    assert lean["no_cycle_word_gapped_three_even_ee"] is True
    assert lean["no_cycle_word_gapped_three_even_eoe"] is True
...
def test_classify_render_and_artifacts():
    ...
    assert "no_cycle_word_gapped_three_even_ee" in text
    from research.juggler_sequence.gapped_cycle_word import JSON_PATH

    assert JSON_PATH.is_file()
    data = json.loads(JSON_PATH.read_text(encoding="utf-8"))
    assert data["experiment"] == "juggler_gapped_cycle_word"
    assert data["decision"]["classification"] == CLASS_GREEN
    assert data["anti_overclaim"]["cycles_impossible"] is False
    assert data["anti_overclaim"]["gapped_cycle_word_lean"] is True
    assert data["lean"]["no_cycle_word_gapped_three_even_ee"] is True
    assert data["lean"]["no_cycle_word_gapped_three_even_eoe"] is True
...
def test_dossier_boundary():
    ...
    assert "no_cycle_word_gapped_three_even_ee" in dossier
    assert "no_cycle_word_gapped_three_even_eoe" in dossier
