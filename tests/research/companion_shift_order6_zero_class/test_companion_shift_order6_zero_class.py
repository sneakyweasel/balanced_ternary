"""Frozen-engine campaign on an order-6 vanishing class constraint."""
...
from research.companion_shift_order6_zero_class.lean_export import LEAN_MODULE, LEAN_PATH, THEOREMS
from research.companion_shift_order6_zero_class.planner import plan_strategy
from research.companion_shift_order6_zero_class.problem import PROBLEM
from research.companion_shift_order6_zero_class.runner import CURRENT, LIVE_ID, run_campaign
from research.companion_shift_order6_zero_class.spec import LAST_ROW, WINDOW, map_spec
...
SRC = ROOT / "src" / "research" / "companion_shift_order6_zero_class"
FORBIDDEN_SPEC = (
    "interpolant",
...
def test_adapter_sources_are_blind_and_do_not_import_scout():
    for name in ("spec.py", "adapter.py", "planner.py"):
        ...
        for line in text.splitlines():
            stripped = line.strip()
            if "companion_shift_order6_zero_class.scout" in stripped:
...
def test_blind_packet_matches_the_stored_board_definition():
    ...
    assert observation(WINDOW) == 12
    assert next_window(WINDOW, LAST_ROW)[0] == WINDOW[1]
    assert skip_attacks_for_dimension(6) == ("vector_affine", "matrix_word_invariant")


def test_problem_descriptor_and_prior_art():
    assert get_problem(CURRENT) is PROBLEM
    assert PROBLEM.docs == ("docs/problems/companion_shift_order6_zero_class.md",)
...
def test_post_run_prefix_and_skipped_lattice():
    spec = map_spec()
    report = evidence_state(spec)
    assert report["zero_at"] is None
    assert report["status"] == "FINITE_ZERO_FREE"
    assert report["first_negative"] == 11
    ...
    assert report["moduli_without_zero"] == ()
    flag = falsify_claims(spec)
    assert flag["prefix_means_nonexistence"]["status"] == "REFUTED"
    assert flag["matrix_word_gives_vanishing_congruence"]["status"] == "REFUTED"
...
def test_campaign_runs_unmodified_loop():
    ...
    assert FailureClass.COMPUTATIONAL.value in classes or FailureClass.GLOBAL_REASONING.value in classes
