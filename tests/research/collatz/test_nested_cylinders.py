...
def test_all_ones_closed_form():
    """R((1,)*m) = 2^{m+1}-1. EXACT — HUMAN PROOF; this is a check of the formula."""
    for m in range(0, 16):
...
def test_certificate_labels():
    ...
    assert names["trivial_upper_bound"] == "EXACT — HUMAN PROOF"
    assert names["all_ones_R_unbounded"] == "EXACT — HUMAN PROOF"
