...
10. [docs/theory/juggler_leverage_prospecting_note.md](docs/theory/juggler_leverage_prospecting_note.md) — prospecting pass of 6 Sep 2026: the visible open objects of the program ranked by an estimated leverage ratio (discovery times documented downstream reach, over cost), with the twelve excluded objects and the standing fences listed. Every ranking is **OBSERVATION** about the corpus, not about the map; the note proves nothing, opens no branch, raises no floor, and its own conclusion is that the surviving candidates are cheap consolidation below the frontier — the high-reach objects are exactly the ones the ratio demotes for near-zero proofability. Do not read it as evidence for termination.
