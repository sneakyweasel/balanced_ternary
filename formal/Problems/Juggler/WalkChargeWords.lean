...
/-!
# Walk-charge words: the exact hug word and the word identity

Paper A Section 5 (the walk-charge envelope) prices the coupled
exponent walk of a minimum-based cycle by the *hug word*: take an
...
/-- Letter of the exact hug word at position `k` (`true` = even). -/
def hugLetter (k : ℕ) : Bool := hugIsEven (hugOdds k) k

/-- The exact hug word of length `L`. -/
...
/-- **Word identity** (Paper A Lemma 5.6, discrete side): the
budgeted hug word at `(L, hugOdds L)` equals the exact hug
