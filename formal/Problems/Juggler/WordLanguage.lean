...
/-!
# Existential Juggler word languages

A word is in `jugglerLanguage` when some integer realises it. The
expanding and persistent-expanding languages keep the same
existential quantifier: they do not say that every realisation
expands. Syntactic `expandingWord` remains a letter-count predicate
...
def wordPrefix (p w : List Branch) : Prop :=
  ∃ s, p ++ s = w

def wordSuffix (s w : List Branch) : Prop :=
  ∃ p, p ++ s = w

def wordFactor (u w : List Branch) : Prop :=
  ∃ p s, p ++ u ++ s = w
...
theorem jugglerLanguage_prefix {p w : List Branch}
    (h : jugglerLanguage w) (hp : wordPrefix p w) : jugglerLanguage p := by
  obtain ⟨n, hw⟩ := h
  obtain ⟨s, hs⟩ := hp
  exact ⟨n, follows_of_append_left (Eq.mp (congrArg (follows n) hs.symm) hw)⟩

theorem jugglerLanguage_suffix {s w : List Branch}
    (h : jugglerLanguage w) (hs : wordSuffix s w) : jugglerLanguage s := by
  obtain ⟨n, hw⟩ := h
  obtain ⟨p, hp⟩ := hs
  exact ⟨image n p,
...
theorem jugglerLanguage_factor {u w : List Branch}
    (h : jugglerLanguage w) (hu : wordFactor u w) : jugglerLanguage u := by
  obtain ⟨n, hw⟩ := h
  obtain ⟨p, s, hps⟩ := hu
...
theorem oe_factor_of_ooe :
    wordFactor (oddEvenBlock 1 1) (oddEvenBlock 2 1) :=
