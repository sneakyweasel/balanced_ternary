...
exact target
  → generic diagnosis (RegimeFingerprint, StructuralDelta, FamilyStatus,
    CapabilityCoverage)
  → attack planner (reconnaissance, piecewise_affine, parameter_domain, then existing order)
...
`latent_control` (`NONE|FINITE|PARAMETERIZED|UNCERTAIN|UNOBSERVED`), and
`parameter_domain` (`UNOBSERVED|SAMPLE_SUPPORTED|EXACT|UNCERTAIN`).
They do not join `CORE_DIMENSIONS`.
...
- `ComplexityProfile` is unchanged. Census and domain counts live on
  `AttackResult.evidence`.
- The census does not install controls, moduli, or `AffineSystem` on
  the spec. Domain certification consumes `prior_results`, not an
  injected affine system.
- No Syracuse-specific engine types. Coefficient search is a documented
  integer box, not a seeded \(3x+1\) rule. Maximal exponent is a
  divisibility conjunction, not a hard-coded valuation hint.
...
`research_engine.attacks.parameter_domain`. CLI: `btlab research analyze`
