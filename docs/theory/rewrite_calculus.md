...
unique syntactic normal form (**PROVED**). Global confluence of the
...
The integer identity `N(D(x)) = D(N(x))` (`rewrite_N_D`) is **not** a
tree rule.
...
- `N(S)`, `N(I-)`, `N(I+)` (and the engine shortcut `N(I0)`) move `N`
  inward past one pushable constructor and drop `N-inversion` by one.
...
or `I0`. Equivalently, after `N` is pushed inward past every `I±`/`S`,
the term is an `I±`/`S` spine over a `D`-safe core:

```text
NF   ::= x | N(Safe) | D(Safe) | I-(NF) | I+(NF) | S(NF)
Safe ::= x | N(Safe) | D(Safe)
```

with the side condition that the argument of an `N` in `Safe` is not
itself an `N` (so `N(N(…))` is excluded). `N` may remain above a
`D`-chain; the tree rules never move `N` past `D`.

## Semantic incompleteness

Unique syntactic NF is not unique representation of integer functions.
`N(D(x))` and `D(N(x))` are both irreducible, distinct as trees, and
agree under `evaluate` on every integer probe
([BTC-op-fragment-semantic-nf](theorem_ledger.md), **REFUTED**). The
Lean identity `rewrite_N_D` explains the agreement; it is not a tree
rule. The same pair already appears as closed terms
`N(D(n))` versus `D(N(n))`.
...
and `rewrite_expr` (innermost-left) matches that descendant. Every
one-step contraction on size `≤ 5` drops the termination rank. The
irreducibles of size `≤ 6` are exactly the grammar above
...
The open-term unique-NF question for this fragment is closed
(**PROVED**). Adding `N(D(x)) → D(N(x))` as a tree rule, and asking
whether the enlarged system remains confluent and becomes a complete
canonical form for the integer operator algebra, is a separate
