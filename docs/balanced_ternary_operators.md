...
**EXACT — HUMAN PROOF**, **COMPUTATIONALLY VERIFIED**, **CONJECTURE**, **OBSERVATION**,
...
**EXACT — HUMAN PROOF.** If \(n=\sum_i a_i 3^i\) then \(3n=\sum_i a_i 3^{i+1}\). For
...
**EXACT — HUMAN PROOF.** \(N(n)=-n\) is digitwise negation. \(N\circ N=\mathrm{id}\).
\(w(N(n))=w(n)\) and \(s(N(n))=-s(n)\). \(N\) commutes with \(S\) and with
\(D\). Lean: `mapNeg_involutive`, `N_commutes_S_word`.

## Digit derivative \(D\)

**EXACT — HUMAN PROOF.** \(a_0\in\{-1,0,+1\}\) is \(n\) reduced modulo 3 into that alphabet,
and
...
**EXACT — HUMAN PROOF.** \(D\circ S=\mathrm{id}\). \(S\circ D(n)=n-a_0\), hence
\(S\circ D=\mathrm{id}\) if and only if \(3\mid n\). \(D\circ N=N\circ D\).
The maps \(I_{\pm}\) are sections: \(D\circ I_{\pm}=\mathrm{id}\).

**EXACT — HUMAN PROOF.** The \(D\)-orbit of \(n\) is \(n,D(n),\ldots,0\). Successive LSDs
...
- **EXACT — HUMAN PROOF:** \(W(W(n))=n\) iff \(n=0\) or \(3\nmid n\). Equivalently
  \(W\circ W=K_3\).
- **EXACT — HUMAN PROOF:** \(W_z\) and \(W_t\) are involutions on \(\mathbb{Z}\).
- **EXACT — HUMAN PROOF:** \(W(-n)=-W(n)\) and \(W(3^m n)=W(n)\), so \(W\circ S=W\).
- **EXACT — HUMAN PROOF:** \(W^3=W\) as functions, because \(W\circ K_3=K_3\circ W=W\).
...
**EXACT — HUMAN PROOF.** \(I_d(n)=3n+d\) for \(d\in\{-1,0,+1\}\) satisfies \(D(I_d(n))=n\).
The three integrals are the complete set of right inverses of \(D\).

## Three-kernel \(K_3\)

**EXACT — HUMAN PROOF.** \(K_3(n)=n/3^{v_3(n)}\) (with \(K_3(0)=0\)) is LSD-first
