...
| Leftover length-six orientations | [Leftover cycle words](problems/juggler_leftover_cycles.md) | PROMOTE | **EXACT — LEAN VERIFIED** | `OOOEOE` and `OOOOEE` are impossible as cycle words; consolidated into the small-cycle census `no_cycle_word_length_le_six` (no cycle of length \(\le6\); length \(\ge7\) open) | Cycle theorem |
