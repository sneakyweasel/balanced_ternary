...
| Two-step parity | [Multi-step itinerary-parity census](problems/juggler_two_step_parity.md) | PROMOTE | **EXACT — HUMAN PROOF** | Exact linearizations remove every nested fractional part; Theorems C and E give the joint parity classes as \(N/8\) and \(N/16\) \(+O(N^{23/24+\varepsilon})\); Corollary F lifts the certified \(\le4\)-step descent class to density \(13/16\) — the exact ceiling of the one-growing-layer method (Proposition I). Second-order bricks for the next tier are proved (`J-second-order-linearization`), and all-depth equidistribution implies density-one finite descent (`J-equidistribution-implies-density-one`, the Juggler Terras program); tier-2 \(OOO\ast\) bound open | Analytic theorem |
