...
| Two-step parity | [Multi-step itinerary-parity census](problems/juggler_two_step_parity.md) | PROMOTE | **EXACT — HUMAN PROOF** | The exact linearization \(m^{3/2}=\tfrac32 mn^{3/4}-\tfrac12 n^{9/4}+O(n^{-3/4})\) removes the nested fractional part, and the reviewed Theorem C gives each joint parity class of \((J(n),J^2(n))\) on odd \(n\le N\) as \(N/8+O(N^{23/24+\varepsilon})\) (`J-nested-parity-discrepancy`); depth-4 OOEE extension open | Analytic theorem |
