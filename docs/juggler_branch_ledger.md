...
| Two-step parity | [Multi-step itinerary-parity census](problems/juggler_two_step_parity.md) | PROMOTE | **EXACT — HUMAN PROOF** | Exact linearizations remove every nested fractional part; Theorems C/E/Q and Proposition L give every word class at depths \(\le4\) except OOO\* \(+O(N^{1-\delta})\); Corollary F lifts the certified \(\le4\)-step descent class to density \(13/16\) — the exact ceiling of the one-growing-layer method (Proposition I). All-depth equidistribution implies density-one finite descent (`J-equidistribution-implies-density-one`, the Juggler Terras program). The OOO\* pair is exactly the isolated kernel \(K_c=\sum e(c(n)\{m^{3/2}\})\), \(c\asymp n^{9/8}\) (Conjecture O, square-root cancellation observed), with a sharp engine/kernel boundary at coefficient growth \(n\). Consolidated: imported into the finite-dynamics note as Section 5 (Theorems 5.4–5.8, Corollary 5.9) and Section 6 (Proposition 6.1, Conjecture 6.2); floor reductions Lean-verified (`GapCells.lean`). Phase 8: a complete double-differencing DRAFT proof of the kernel bound (Theorem R, \(\delta=1/72\) for \(k\le P^{1/24}\)) with exact bricks Lemmas R1–R3 machine-validated and `seq_floor_gap_second` in Lean; tag stays CONJECTURE pending adversarial review | Analytic theorem |
