...
| Two-step parity | [Multi-step itinerary-parity census](problems/juggler_two_step_parity.md) | PROMOTE | **EXACT — HUMAN PROOF** | Exact linearizations remove every nested fractional part; Theorems C/E and Propositions L give all word classes at depths \(\le3\) plus the OOE\* splits at depth 4 \(+O(N^{1-\delta})\); Corollary F lifts the certified \(\le4\)-step descent class to density \(13/16\) — the exact ceiling of the one-growing-layer method (Proposition I). All-depth equidistribution implies density-one finite descent (`J-equidistribution-implies-density-one`, the Juggler Terras program). The tier-2 \(OOO\ast\) reduction is driven to a single named obstruction: the kernel \(K_c=\sum e(c(n)\{m^{3/2}\})\), \(c\asymp n^{9/8}\) (Conjecture O, square-root cancellation observed); OEO\* at depth 4 promoted as the easier next split | Analytic theorem |
