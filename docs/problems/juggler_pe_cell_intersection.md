...
empty-odd-cell forward laws, and not a claim that every positive
integer reaches 1.

The parked empty-odd-cell branch already named Type 0/1/2 and
...
Let \(y\) be an odd PE landing produced by an exact finite word
...
- `odd_cell_unique` / even and odd floor cells —
  **EXACT — LEAN VERIFIED**
...
- Probe: `research.juggler_sequence.pe_cell_intersection`
- Records: [juggler_pe_cell_intersection.md](../research/juggler_pe_cell_intersection.md),
  [juggler_pe_cell_intersection.json](../research/juggler_pe_cell_intersection.json)
- Tests: `tests/research/juggler_sequence/test_pe_cell_intersection.py`
...
No new Lean module. `odd_cell_unique`, `even_cell_iff`, and the
cube-interval lemmas stay in `Cells.lean`. `AboveAnchor` stays in
`MinimumRelative.lean`. Not imported by `Problems.JugglerPaper`.
No `sorry`. No `PECellIntersection` API. No `juggler_reaches_one`.
The Type 0/1/2 criterion stays in `empty_odd_cell.py`.

## Results

Classification **PE_CELL_INTERSECTION_PARK**.
...
the PE predecessor is even. That is `even_cell_iff` versus
`odd_cell_iff`, not a minimum-relative theorem. Falsifiers A, B
